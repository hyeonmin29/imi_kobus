...
try {
	...
	$nBusMoveInfoSeq = $_POST['bus_move_info_seq'];				# 변경할 버스 정보
	$nBeforeReservationSeq = $_POST['before_reservation_seq'];	# 변경하기 전 버스 예매 정보
	...
	/* 출발위치 */
	if ($rgSelectBus['bus_leave_location'] == 'jeonju') {
		$strReserBusLeaveLocation = '전주';
	}
	if ($rgSelectBus['bus_leave_location'] == 'incheon') {
		$strReserBusLeaveLocation = '인천';
	}
	if ($rgSelectBus['bus_leave_location'] == 'seoul') {
		$strReserBusLeaveLocation = '서울';
	}
	
	/* 도착위치 */
	if ($rgSelectBus['bus_arrive_location'] == 'jeonju') {
		$strReserBusArriveLocation = '전주';
	}
	if ($rgSelectBus['bus_arrive_location'] == 'incheon') {
		$strReserBusArriveLocation = '인천';
	}
	if ($rgSelectBus['bus_arrive_location'] == 'seoul') {
...
<html>
	<body align = 'center'>
		...
		<table align = 'center' border = 1 cellspacing = "0">
			<tr><th>버스번호</th> <th>버스 등급</th> <th>금액</th> <th>버스출발위치</th> <th>버스도착위치</th> <th>잔여 좌석 수</th> <th>운행시간</th> <th>출발일시</th>  <th>도착일시</th></tr>
				<tr>
					...
					<td align = 'center'><?=$strReserClass?></td>
					<td align = 'center'><?=number_format($rgSelectBus['price'])?></td>
					<td align = 'center'><?=$strReserBusLeaveLocation?></td>
					<td align = 'center'><?=$strReserBusArriveLocation?></td>
					<td align = 'center'><?=$rgSelectBus['seat_cnt'] - $rgSelectBus['people_cnt']?></td>
		...
		<form method = 'POST' action = '../bus/BusChangeOk.php'>
				<!--프리미엄 및 우등 좌석 배치-->
				<p>
						...
						for ($nCol = 1; $nCol<10; $nCol++) {
							...
							for ($nRow = 1; $nRow<5; $nRow++) {
								if($rgSelectBus['bus_class'] !== 'n' & $nRow==3 & $nCol<9 ){
									...
									?>
									<td><input type = 'radio' name = 'seat_num' width="25%" value = '<?=$nSeatNum?>' <?=$disabled?>> <?=$nSeatNum?> </td>
									<?
									$nSeatNum++;
								}
							}
			...
			<input type = 'hidden' name = 'after_bus_move_info_seq' value = <?=$nBusMoveInfoSeq?>>
